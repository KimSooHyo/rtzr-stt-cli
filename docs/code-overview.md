...
초기 검토에서는 checkpoint, 완료 cache, output lock, upload snapshot, 디렉터리 `fsync` 같은 운영 기능까지 범위에 넣었다. 그러나 이 프로젝트는 배포 서비스가 아니라 한 사람이 로컬에서 실행하고 이해하는 CLI다. 학습·검토 비용에 비해 범위를 넘는 복잡성이 커서 최종 코드에서는 제외했다.
